...
packages/renderer/src/
  ...
  core/                       # Framework-agnostic engine
    ...
    plugins.ts                # Runs a plugin list's hooks as one: geometry in sequence, paint as a chain of next() ending in paintStroke; plugin `steps` (a cycle of drawings: stepAt / pluginStepsAt / allPluginSteps)
...
The inspector's **Plugins** tab (Text mode) switches on the shipped `variationPlugin` (issue #31) and `boilPlugin`, and demo plugins that show off the renderer's `TegakiPlugin` API, written the way a user of `tegaki` would: one or more per hook: slant, a shaky hand and a broad calligraphy nib (geometry); practice paper — ruled lines, 田字格 / 米字格 squares or graph paper, laid out by each stroke's `place`, with a tracing guide (underlay); stroke-order numbers and arrows kept clear of the ink (underlay + overlay); palette colors, a ballpoint that skips, graphite with dust and splinters, wet ink that pools and dries lighter behind the pen (from the paint context's `time`), a bristly brush that becomes a great scroll brush (Size / Press / Splatter), and echo passes (paint); neon that sputters and hums (paint-only `steps` as its clock, `idle` to flicker on); a laser that writes with a beam and a cooling line; paper grain, ink bleed and a drop shadow with an emboss bevel (ink); sparkles, a karaoke ball and bar, and a fountain pen / pencil / quill at the pen head (overlay); a pencil scratch sound and phone haptics (onFrame). Each is made with `createPlugin` ([createPlugin.ts](packages/renderer/src/core/createPlugin.ts)), and the tab builds its controls — preset chips, then a DialKit control per param — from the factory's `params` / `presets`, so a new param or preset shows up in the UI with no panel code. They live in [components/plugins/](packages/website/src/components/plugins) and are only for showing: the `pg` param (which are on) and `po` (their changed options) carry them to `/preview`, but Export and Ask an agent leave them out.
...
| `pg` | Plugins switched on, comma-separated (`vary`, `boil`, `slant`, `shaky`, `nib`, `paper`, `order`, `colors`, `ball`, `graphite`, `wet`, `neon`, `laser`, `brush`, `echo`, `grain`, `bleed`, `shadow`, `sparkle`, `karaoke`, `pen`, `sound`, `haptics`) | `pg=vary,pen` |
...
| `rs` | The renderer's `seed` (unset = 0): wobble phase, gradient hue, what Variation, Boil and the random demos (Brush, Ballpoint, Shaky hand, Graphite, Neon's flicker, Sparkles, shuffled Colors) draw by | `rs=42` |

---
title: React
description: 'Add a handwriting animation to a React or Next.js app with the TegakiRenderer component: controlled time, effects, streaming text and the imperative handle.'
head:
  - tag: title
    content: 'React Handwriting Animation | Tegaki'
---
...
If the text doesn't draw in `vite dev` (you can select it, or see it with `showOverlay`, but nothing animates), add `optimizeDeps: { exclude: ['tegaki'] }` to `vite.config.ts` and restart the dev server. See [Bundler Setup](/tegaki/guides/bundlers/#vite).
:::

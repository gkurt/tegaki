...
description: 'Animate handwriting in any page with the <tegaki-renderer> custom element: load it from a CDN, control playback with attributes, and drive it with CSS.'
head:
  - tag: title
    content: 'Handwriting Animation Web Component | Tegaki'
---
...
This is also useful for loading custom font bundles generated with the [studio](/tegaki/studio/).
...
| `reduced-motion` | `"never" \| "user" \| "always"` | Whether to honour reduced motion by showing the text finished (default `"never"`; see [`reducedMotion`](/tegaki/api/renderer/#reducedmotion)). |
...
| `seed` | `number \| "random"` | What random choices come from: wobble, gradients, plugins (default `0`; see [`seed`](/tegaki/api/renderer/#seed)). The element's `engine.seed` is the number it drew with. |
| `show-overlay` | _boolean attribute_ | Show debug text overlay. |
| `fallback-font` | `string` | CSS font-family list for characters the bundle has no stroke data for (see [`fallbackFont`](/tegaki/api/renderer/#fallbackfont)). |

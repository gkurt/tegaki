...
function keyFacts(): string[] {
  return [
    ...
    '- Effects: glow, wobble, pressure width, taper, per-stroke and whole-text gradients; custom `TegakiPlugin`s can reshape and paint strokes, and `createPlugin` gives one typed options and presets. `variationPlugin` makes each glyph a little different; a fixed `seed` (default 0) draws the same every time, `seed: "random"` anew on each load.',
  ];
}

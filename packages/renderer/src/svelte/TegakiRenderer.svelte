...
let { text, font, time: timeProp, reducedMotion, onComplete, onChangeTimeline, effects, quality, timing, showOverlay, direction, shaper, fallbackFont, class: className, style: userStyle, ...attrs }: Props = $props();
...
const { rootProps, content: innerHtml } = TegakiEngine.renderElements(
  { text, font, time: timeProp, reducedMotion, effects: effects as Record<string, any>, quality, timing, showOverlay, direction, shaper, fallbackFont, onComplete, onChangeTimeline },
  svelteCreateElement,
);
